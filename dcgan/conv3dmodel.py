...
from dcgan.dense_layer import AVGPool3DConcatDenseLayer, AVGPool3DDenseClassifer, SimpleOneLayerClassifer
...
class Conv3DGenerator(nn.Module):
    def __init__(self, params):
        ...
        self.layers = nn.Sequential(
            ...
            Conv3DEncoderBlock(params, in_channel=params['nc'], channels=[ 32, 64, 128, 256, 512]),
...
class Conv3DEncoderBlock(nn.Module):
    ...
    def __init__(self, params, in_channel=1, channels=[32, 64, 128]) -> None:
        ...
        for c_out in channels:
            self.layers.append(
                ConvBlock(c_in, c_out, 4, padding="same")
            )
            c_in = c_out
    ...
    def __init__(self, params):
        ...
        self.layers = nn.Sequential(
                ...
                ConvBlock(
                    params['nc'],
                    ndf,
                    kernel_size=4, act=t.rrelu, batchnorm=True, padding="same"),
                ...
                ConvBlock(
                    8 * ndf,
                    1,
                    kernel_size=4, act=t.rrelu, batchnorm=False, padding="same"),

                # nn.Flatten(),
                # nn.Linear( 81920, 1),
                # nn.Sigmoid()
            # AVGPool3DConcatDenseLayer( params, 1 ,params['in_seq_len'] + params['out_seq_len'], params['imsize'], 64),
            SimpleOneLayerClassifer(params, (1* (params['in_seq_len'] + params['out_seq_len']) * params['imsize'] * params['imsize'] ) ),
...
class Conv3DFrameDiscriminator(nn.Module):
    def __init__(self, params):
        ...
        self.layers = nn.Sequential(
            ...
            ConvBlock(params['nc'] , params['ndf'], 4, act=t.rrelu, batchnorm=True, padding="same"),
            ConvBlock(params['ndf'], params['ndf'] * 2, 4, act=t.rrelu, batchnorm=True, padding="same"),
            ConvBlock(params['ndf'] * 2, params['ndf'] * 4, 4, act=t.rrelu, batchnorm=True, padding="same"),
            ConvBlock(params['ndf'] * 4, params['ndf'] * 8,4, act=t.rrelu, batchnorm=True, padding="same"),
            ConvBlock(params['ndf'] * 8, 1 , 4, act=t.rrelu, batchnorm=False, padding='same'),
            SimpleOneLayerClassifer(params, (1 * ( params['out_seq_len']) * params['imsize'] * params['imsize'] ) ),
        )
    def forward(self, x):
        x = x.permute(0,2,3,4,1)
        # ipdb.set_trace()
