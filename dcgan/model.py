...
class Generator(nn.Module):
    def __init__(self, params):
        ...
        self.bn1 = nn.BatchNorm2d(params["nc"] * 8)

        # Input Dimension: (ngf*8) x 4 x 4
        self.tconv2 = nn.Conv2d(
            params["nc"] * 8, params["nc"] * 4, 4, padding="same"
        )
        self.bn2 = nn.BatchNorm2d(params["nc"] * 4)

        # Input Dimension: (ngf*4) x 8 x 8
        self.tconv3 = nn.Conv2d(
            params["nc"] * 4, params["nc"] * 2, 4, padding="same"
        )
        self.bn3 = nn.BatchNorm2d(params["nc"] * 2)

        # Input Dimension: (ngf*2) x 16 x 16
        self.tconv4 = nn.Conv2d(
            params["nc"] * 2, params["nc"], 4, padding="same"
        )
        self.bn4 = nn.BatchNorm2d(params["nc"])

        # Input Dimension: (ngf) * 32 * 32
        self.tconv5 = nn.Conv2d(params["nc"], params["nc"], 4, padding="same")
...
class TemporalDiscriminator(nn.Module):
    def __init__(self, params):
        ...
        self.conv1 = nn.Conv2d(
            2 * params["nc"], params["ndf"], 4, 2, 1, bias=False
        )

        # Input Dimension: (ndf) x 32 x 32
        self.conv2 = nn.Conv2d(
            params["ndf"], params["ndf"] * 2, 4, 2, 1, bias=False
        )
...
class FrameDiscriminator(nn.Module):
    def __init__(self, params):
        ...
        # Input Dimension: (nc) x 64 x 64
        self.conv1 = nn.Conv2d(
            params["nc"], params["ndf"], 4, 2, 1, bias=False
        )
