...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            ...
            *block(256, 512),
            *block(512, 1024),
            nn.Linear(1024, int(t.prod(t.tensor(img_shape)))),
            nn.Tanh()
        )
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            nn.Linear(int(t.prod(t.tensor(img_shape))), 512),
...
def main():
    ...
    for epoch in range(opt.n_epochs):
        for i, (imgs, _) in enumerate(dataloader):
            ...
            z = t.randn((imgs.shape[0], opt.latent_dim)).to(device)
