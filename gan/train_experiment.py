...
def train_single_epoch(
    *,
    ...
    for i, (x, y) in enumerate(dataloader):
        ...
        if i == 0:
            pred_y = netG(x).cpu()
            visualize_predictions(x, y, pred_y, epoch)
        ...
        if i % 50 == 0:
            ...
            t.save(
                netG.state_dict(),
                os.path.join(curdir, "models", "netG_epoch_" + str(epoch) + ".pth"),
            )
            t.save(
                netFD.state_dict(),
                os.path.join(curdir, "models", "netFD_epoch_" + str(epoch) + ".pth"),
            )
            t.save(
                netTD.state_dict(),
                os.path.join(curdir, "models", "netTD_epoch_" + str(epoch) + ".pth"),
            )
