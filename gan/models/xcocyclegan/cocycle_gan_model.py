...
class CoCycleGAN(LightningModule):
    ...
    def forward(self, x: t.Tensor, future: bool):
        return self.generator(self.__embed(x, future), future=future)
    ...
    def __visualize_predictions(self, x, y, flag):
        ...
        fake_y = self.forward(x, future=future)
        fake_x = self.forward(fake_y, future=not future)

        _visualize_predictions(
            x,
            y,
            fake_y,
            self.forward(y, future=self.x_future),
            fake_x,
            self.current_epoch,
    ...
    def __one_way_generator_loss(
        ...
        if (future and self.params.backward_generator_loss) or (
            ...
            fake_y = self.forward(x, future=future)
            fake_x = self.forward(y, future=not future)
    ...
    def __one_way_discriminator_loss(self, x: t.Tensor, y: t.Tensor, future: bool):
        ...
        fake_y = self.forward(x if future else y, future=future)
        if future:
    ...
    def __cyclic_way_discriminator_loss(self, x: t.Tensor, y: t.Tensor):
        ...
        fake_y = self.forward(x, future=self.y_future)
        fake_x = self.forward(fake_y, future=self.x_future)
    ...
    def __all_way_discriminator_loss(self, x: t.Tensor, y: t.Tensor):
        ...
        fake_y = self.forward(x, future=True)
        fake_x = self.forward(fake_y, future=False)
        pred_x = self.forward(y, future=False)
    ...
    def __generator_loss(self, x: t.Tensor, y: t.Tensor, batch_idx: int, flag: str):
        ...
        pred_y = self.forward(x, future=self.y_future)
        fake_x = self.forward(pred_y, future=self.x_future)
        pred_x = self.forward(y, future=self.x_future)
        cycle_y = self.forward(pred_x, future=self.y_future)
