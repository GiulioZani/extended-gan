...
class CoCycleGAN(LightningModule):
    ...
    def __generator_loss(self, x: t.Tensor, y: t.Tensor, batch_idx: int, flag: str):
        ...
        l2_lambda = self.params.hparams.l2_norm_weight
    ...
    def __frame_discriminator_loss(self, x: t.Tensor, y: t.Tensor):

        b, ts, c, h, w = x.shape
        all = t.cat((x, y), dim=1)
        all = all.view(b * 2 * ts, c, h, w)

        fake_y = self.generator(self.__embed(x, future=self.y_future))
        fake_x = self.generator(self.__embed(y, future=self.x_future))

        all_fake = t.cat((fake_x, fake_y), dim=1)
        all_fake = all_fake.view(b * 2 * ts, c, h, w)

        # select b random frames from all_fake
        rand_perm = t.randperm(all_fake.shape[0])[:b]
        all_fake = all_fake[rand_perm]
        all = all[rand_perm]

        pred_x = self.frame_discriminator(all)
        pred_y = self.frame_discriminator(all_fake)
        true_label = t.ones(all.shape[0], 1, device=self.device)
        fake_label = t.zeros(all.shape[0], 1, device=self.device)
        return (
            self.__adversarial_loss(pred_x, true_label)
            + self.__adversarial_loss(pred_y, fake_label)
        ) / 2

    def __frame_disc_generator_adversarial_loss(self, x: t.Tensor, y: t.Tensor):
        ...
        all_fake = t.cat((fake_x, fake_y), dim=1)
        all_fake = all_fake.view(
            all_fake.shape[0] * all_fake.shape[1], *all_fake.shape[2:]
        )

        # select b randomly from all_fake
        all_fake = all_fake[t.randperm(all_fake.shape[0])[:b]]

        pred_x = self.frame_discriminator(all_fake)
        true_label = t.ones(all_fake.shape[0], 1, device=self.device)

        return self.__adversarial_loss(pred_x, true_label)
    ...
    def training_step(
        ...
        if optimizer_idx == 2:
            loss = self.__frame_discriminator_loss(x, y)
            self.log("FD_Loss", loss, True)
            return {
                "loss": loss,
            }
    ...
    def configure_optimizers(self):
        ...
        return [
            ...
            {
                "optimizer": frame_discriminator_optimizer,
            },
        ]
