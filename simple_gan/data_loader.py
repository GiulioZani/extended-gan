...
class DataLoader:
    ...
    def __next__(self) -> tuple[t.Tensor, t.Tensor]:
        ...
        if len(result) == 0:
            raise StopIteration
        result = t.stack(
            tuple(t.stack((s[:4], s[4:])) for s in result)
        ).transpose(0, 1)
        rand_indices = (
        ...
        results = (
            result[0][rand_indices].to(self.device),
            result[1][rand_indices].to(self.device),
        )
        return results
...
def get_loaders(
    ...
    return (
        DataLoader(train_folder, train_batch_size, device),
        DataLoader(test_folder, test_batch_size, device),
    )
